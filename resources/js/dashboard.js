document.addEventListener("DOMContentLoaded", () => {
    ...
    function openDetailModal(barangId) {
        ...
        const modalTglPenyerahan =
            document.getElementById("modalTglPenyerahan");
        const modalTglPengembalian = document.getElementById(
            "modalTglPengembalian"
        );
        ...
        if (!modalDeviceName || !deviceImage) {
            console.error(
                "Elemen wajib pada modal detail (seperti nama atau gambar) tidak ditemukan."
            );
            return;
        }
        ...
        fetch(`/barang/detail/${barangId}`)
            ...
            .then((data) => {
                if (data.success && data.barang) {
                    ...
                    modalDeviceType.textContent = barang.jenis_barang
                        ? barang.jenis_barang.nama_jenis
                        : "N/A";
                    modalPerusahaan.textContent = barang.perusahaan
                        ? barang.perusahaan.nama_perusahaan
                        : "N/A";
                    modalNoAsset.textContent = barang.no_asset || "N/A";
                    modalKuantitas.textContent = barang.kuantitas || "N/A";
                    modalTglPengadaan.textContent = formatDate(
                        barang.tgl_pengadaan
                    );
                    modalSerialNumber.textContent =
                        barang.serial_number || "N/A";
                    modalLokasi.textContent = barang.lokasi || "N/A";

                    if (track) {
                        modalUser.textContent = track.username || "N/A";
                        lastKnownUserFromDetail = track.username || "-";
                        modalTglPenyerahan.textContent = formatDate(
                            track.tanggal_awal
                        );
                        modalTglPengembalian.textContent = track.tanggal_ahir
                            ? formatDate(track.tanggal_ahir)
                            : "-";
                        modalStatus.innerHTML = `<span style="font-weight: bold; color: var(--color-accent);">${
                            track.status || "N/A"
                        }</span>`;
                        modalKeterangan.textContent =
                            track.keterangan || "Tidak ada keterangan.";
                    } else {
                    ...
                    const btnUpdateAset = document.querySelector(
                        "#deviceInfoModal .btn-action.btn-update"
                    );
                    if (btnUpdateAset) {
                        const currentStatus = track
                            ? track.status
                            : barang.status;
                        ...
                        if (currentStatus === "non aktif") {
                            ...
                            btnUpdateAset.dataset.serialNumber =
                                barang.serial_number;
                            btnUpdateAset.onclick = handleOpenSerahTerimaModal;
                        }
                    }

                    const triggerHistoryButton = document.getElementById(
                        "triggerUserHistoryModalButton"
                    );
                    if (triggerHistoryButton && barang.serial_number) {
                        const newTriggerHistoryButton =
                            triggerHistoryButton.cloneNode(true);
                        triggerHistoryButton.parentNode.replaceChild(
                            newTriggerHistoryButton,
                            triggerHistoryButton
                        );
                        const deviceFullName = `${barang.merek || "N/A"} (${
                            barang.jenis_barang
                                ? barang.jenis_barang.nama_jenis
                                : "N/A"
                        })`;
                        const currentCompany = barang.perusahaan
                            ? barang.perusahaan.nama_perusahaan
                            : "N/A";
                    ...
                    deviceImage.className =
                        "device-image fas fa-exclamation-triangle";
                }
            })
            .catch((error) => {
                console.error(
                    "Error selama proses fetch detail barang:",
                    error
                );
                alert(
                    "Gagal mengambil detail barang. Lihat konsol untuk detail."
                );
                modalDeviceName.textContent = "Error";
                deviceImage.className =
                    "device-image fas fa-exclamation-triangle";
            ...
            if (isNaN(date.getTime())) {
                ...
                if (parts.length === 3) {
                    ...
                    if (!isNaN(parsedDate.getTime())) {
                        ...
                        const m = String(parsedDate.getMonth() + 1).padStart(
                            2,
                            "0"
                        );
    ...
    function handleSerahTerimaStatusChange() {
        const serahTerimaStatusSelect =
            document.getElementById("serahTerimaStatus");
        const serahTerimaUserInput = document.getElementById("serahTerimaUser");
        const perusahaanTujuanGroup = document.getElementById(
            "perusahaanTujuanGroup"
        );
        const serahTerimaPerusahaanTujuanSelect = document.getElementById(
            "serahTerimaPerusahaanTujuan"
        );
        if (
            !serahTerimaStatusSelect ||
            !serahTerimaUserInput ||
            !perusahaanTujuanGroup ||
            !serahTerimaPerusahaanTujuanSelect
        ) {
            return;
        }
        ...
        const perusahaanTujuanErrorEl = document.getElementById(
            "perusahaan_tujuan_serah_error"
        );
        ...
        const previousUser =
            lastKnownUserFromDetail &&
            lastKnownUserFromDetail !== "-" &&
            lastKnownUserFromDetail.trim() !== ""
                ? lastKnownUserFromDetail
                : "User Sebelumnya Tidak Diketahui";
        ...
        if (selectedStatus === "digunakan") {
            ...
            serahTerimaPerusahaanTujuanSelect.setAttribute(
                "required",
                "required"
            );
        }
    ...
    async function handleOpenSerahTerimaModal(event) {
        ...
        try {
            ...
            const modalTitle = serahModal.querySelector(
                ".morph-modal-title span"
            );

            modalTitle.innerHTML =
                '<i class="fas fa-exchange-alt"></i> Serah Terima Aset';

            serahForm.reset();

            document.getElementById("serahTerimaTrackId").value = "";
            document.getElementById("serahTerimaAssetId").value = assetId;
            document.getElementById("serahTerimaSerialNumber").value =
                barang.serial_number;
            document.getElementById("serahTerimaInfoNamaAset").textContent =
                barang.merek;
            document.getElementById("serahTerimaInfoSN").textContent =
                barang.serial_number;
            document.getElementById("serahTerimaInfoPerusahaan").textContent =
                barang.perusahaan ? barang.perusahaan.nama_perusahaan : "N/A";
            document.getElementById("serahTerimaTanggalAwal").value = new Date()
                .toISOString()
                .slice(0, 10);
            document.getElementById("serahTerimaTanggalAwal").readOnly = true;

            const perusahaanTujuanSelect = document.getElementById(
                "serahTerimaPerusahaanTujuan"
            );
            ...
            document.getElementById("perusahaanTujuanGroup").style.display =
                "none";
            document.getElementById("noAssetBaruPreviewGroup").style.display =
                "none";
        ...
        fetch(`/track/${trackId}/edit`)
            ...
            .then((data) => {
                if (data.success) {
                    ...
                    const serahModal = document.getElementById(
                        "serahTerimaAsetModal"
                    );
                    const serahForm = document.getElementById(
                        "serahTerimaAsetForm"
                    );
                    const modalTitle = serahModal.querySelector(
                        ".morph-modal-title span"
                    ...
                    modalTitle.innerHTML =
                        '<i class="fas fa-edit"></i> Edit Riwayat Aset';

                    serahForm.reset();
                    document.getElementById("serahTerimaTrackId").value =
                        track.id;
                    document.getElementById("serahTerimaAssetId").value =
                        barang.id;
                    document.getElementById("serahTerimaSerialNumber").value =
                        barang.serial_number;

                    document.getElementById(
                        "serahTerimaInfoNamaAset"
                    ).textContent = barang.merek;
                    document.getElementById("serahTerimaInfoSN").textContent =
                        barang.serial_number;
                    document.getElementById(
                        "serahTerimaInfoPerusahaan"
                    ).textContent = barang.perusahaan.nama_perusahaan;

                    document.getElementById("serahTerimaTanggalAwal").value =
                        track.tanggal_awal;
                    document.getElementById(
                        "serahTerimaTanggalAwal"
                    ).readOnly = false;

                    document.getElementById("serahTerimaUser").value =
                        track.username;
                    document.getElementById("serahTerimaStatus").value =
                        track.status;
                    document.getElementById("serahTerimaKeterangan").value =
                        track.keterangan;
    ...
    function openUserHistoryModal(serialNumber, deviceName, company) {
        ...
        const historyModalSerialNumberEl = document.getElementById(
            "historyModalSerialNumber"
        );
        const historyModalDeviceNameEl = document.getElementById(
            "historyModalDeviceName"
        );
        const historyModalCompanyEl = document.getElementById(
            "historyModalCompany"
        );
        const historyTableBodyEl = document.getElementById(
            "userHistoryTableBody"
        );
        const historyTableHeaderEl = document.getElementById(
            "userHistoryTableHeader"
        );

        const refreshHistory = () =>
            openUserHistoryModal(serialNumber, deviceName, company);
                        ...
                        data.history.forEach((item) => {
                            ...
                            tableRowsHTML += `
                                <tr data-track-id="${item.id}">
                                    <td class="cell-history-user" title="${escapeHtml(
                                        item.username || "-"
                                    )}">${escapeHtml(item.username || "-")}</td>
                                    <td class="cell-history-tgl-awal">${formatDate(
                                        item.tanggal_awal
                                    )}</td>
                                    <td class="cell-history-tgl-akhir">${formatDate(
                                        item.tanggal_ahir
                                    )}</td>
                                    <td class="cell-history-status" title="${escapeHtml(
                                        item.status || "-"
                                    )}">${escapeHtml(item.status || "-")}</td>
                                    <td class="cell-history-keterangan" title="${escapeHtml(
                                        item.keterangan || "-"
                                    )}">${escapeHtml(
                                item.keterangan || "-"
                            )}</td>
    ...
    const productTableRowsContainer = document.getElementById(
        "productTableRowsContainer"
    );
    const paginationContainer = document.getElementById(
        "realtimePaginationContainer"
    );
    const filterPerusahaanSelect = document.getElementById("filter_perusahaan");
    const filterJenisBarangSelect = document.getElementById(
        "filter_jenis_barang"
    );
    ...
    const filterMenu = document.querySelector(
        ".filter-button-wrapper .filter-menu"
    );
    ...
    const resetFilterInMenuBtn = document.getElementById(
        "resetFilterInMenuBtn"
    );

    const addAssetModal = document.getElementById("addAssetModal");
    const openAddAssetModalButton = document.getElementById(
        "openAddAssetModalButton"
    );
    const closeAddAssetModalBtn = document.getElementById(
        "closeAddAssetModalBtn"
    );
    const cancelAddAssetModalBtn = document.getElementById(
        "cancelAddAssetModalBtn"
    );
    const addAssetForm = document.getElementById("addAssetForm");
    const submitAddAssetBtn = document.getElementById("submitAddAssetBtn");

    const detailModalOverlayElement =
        document.getElementById("deviceInfoModal");
    const closeDetailModalButtonElement = document.getElementById(
        "closeDetailModalButton"
    );

    const serahTerimaAsetModalElement = document.getElementById(
        "serahTerimaAsetModal"
    );
    const closeSerahTerimaBtn = document.getElementById(
        "closeSerahTerimaAsetModalButton"
    );
    const cancelSerahTerimaBtn = document.getElementById(
        "cancelSerahTerimaAsetModalBtn"
    );
    const serahTerimaForm = document.getElementById("serahTerimaAsetForm");
    const submitSerahTerimaBtn = document.getElementById(
        "submitSerahTerimaAsetBtn"
    );
    const serahTerimaStatusSelect =
        document.getElementById("serahTerimaStatus");

    const userHistoryModalElement = document.getElementById("userHistoryModal");
    const closeUserHistoryBtn = document.getElementById(
        "closeUserHistoryModalButton"
    );
    ...
    const closeEditAssetModalBtn = document.getElementById(
        "closeEditAssetModalBtn"
    );
    const cancelEditAssetModalBtn = document.getElementById(
        "cancelEditAssetModalBtn"
    );
    ...
    const conveyorContainer = document.querySelector(
    ...
    if (closeEditAssetModalBtn)
        closeEditAssetModalBtn.addEventListener("click", closeEditAssetModal);
    if (cancelEditAssetModalBtn)
        cancelEditAssetModalBtn.addEventListener("click", closeEditAssetModal);
    ...
    if (productTableRowsContainer) {
        productTableRowsContainer.addEventListener("click", function (event) {
                ...
                fetch(`/barang/${assetId}/edit`)
                    ...
                    .then((data) => {
                        if (data.success) {
                            ...
                            document.getElementById(
                                "edit_perusahaan_id"
                            ).value = barang.perusahaan_id;
                            document.getElementById(
                                "edit_jenis_barang_id"
                            ).value = barang.jenis_barang_id;
                            document.getElementById("edit_no_asset").value =
                                barang.no_asset;
                            document.getElementById("edit_merek").value =
                                barang.merek;
                            document.getElementById("edit_kuantitas").value =
                                barang.kuantitas;
                            document.getElementById(
                                "edit_tgl_pengadaan"
                            ).value = barang.tgl_pengadaan;
                            document.getElementById(
                                "edit_serial_number"
            ...
            if (removeButton) {
                ...
                if (
                    confirm(
                        "Apakah Anda yakin ingin menghapus aset ini? Riwayat serah terima yang terkait juga akan dihapus."
                    )
                ) {
    ...
    const updateFilterState = () => {
        ...
        const perusahaan = filterPerusahaanSelect
            ? filterPerusahaanSelect.value
            : "";
        const jenisBarangAktif =
            document.querySelectorAll(
                '#filter_jenis_barang_container input[type="checkbox"]:checked'
            ).length > 0;
    ...
    function manageConveyor() {
        ...
        if (
            ...
            conveyorContainer.scrollWidth <= conveyorContainer.clientWidth
        ) {
            return;
        }

        const originalItems = conveyorContainer.querySelectorAll(
            ".summary-box:not(.summary-box-clone)"
        );
        ...
        const originalSetWidth =
            Array.from(originalItems).reduce(
    ...
    function filterSummaryBoxes(visibleAssetTypes = []) {
        ...
        const allOriginalBoxes = conveyorContainer.querySelectorAll(
            ".summary-box:not(.summary-box-clone)"
        );
        updateFilterState();
        if (!isFilterActive) {
    ...
    const userHistoryTableContainer = document.getElementById(
        "userHistoryTableContainer"
    );
    if (userHistoryTableContainer) {
        userHistoryTableContainer.addEventListener("click", function (event) {
            ...
            if (removeButton) {
                ...
                if (
                    confirm(
                        "Anda yakin ingin menghapus riwayat ini? Tindakan ini tidak dapat diurungkan."
                    )
                ) {
                    fetch(`/track/${trackId}`, {
                        method: "DELETE",
                        ...
                        .then((data) => {
                            if (data.success) {
                                ...
                                alert(
                                    data.message || "Gagal menghapus riwayat."
                                );
                            }
                        })
                        .catch((error) => console.error("Error:", error));
    ...
    function setupAssetPreview() {
        ...
        if (
            ...
            console.error(
                "Satu atau lebih elemen form untuk preview tidak ditemukan. Periksa ID HTML."
            );
            return;
        }

        const updateNoAssetPreview = async () => {
            const perusahaanOption =
                perusahaanSelect.options[perusahaanSelect.selectedIndex];
            const jenisBarangOption =
                jenisBarangSelect.options[jenisBarangSelect.selectedIndex];
            const tglPengadaan = tglPengadaanInput.value;

            if (
                perusahaanOption.value &&
                jenisBarangOption.value &&
                tglPengadaan
            ) {
                ...
                try {
                    ...
                    const response = await fetch(
                        `${window.PAGE_DATA.urls.nextSerial}/${perusahaanId}`
                    );
                    ...
                    if (data.success) {
                        ...
                        const singkatanJenis =
                            jenisBarangOption.dataset.singkatan;

                        if (singkatanPT && singkatanJenis) {
                            const tahun = new Date(tglPengadaan).getFullYear();
                            const bulan = (
                                "0" +
                                (new Date(tglPengadaan).getMonth() + 1)
                            ).slice(-2);
                            ...
                            noAssetPreviewInput.value =
                                "Error: data singkatan tidak ditemukan.";
                        }
                    } else {
                        throw new Error(
                            data.error || "Gagal mengambil nomor seri dari API."
                        );
                    }
                } catch (error) {
                ...
                noAssetPreviewInput.value =
                    "-- Pilih Perusahaan, Jenis, dan Tanggal --";
                if (noAssetWarning) noAssetWarning.style.display = "none";
            }
        };
        ...
        const updateDisplay = () => {
            ...
            if (selected.length === 0) {
                ...
                displaySpan.textContent =
                    selected[0].parentElement.querySelector("span").textContent;
            } else {
    ...
    function renderPaginationControls(paginationData) {
        ...
        buttonsHTML += `<button class="pagination-btn" data-page="${
            currentPage - 1
    ...
    function setupSerahTerimaPreview() {
        ...
        const noAssetBaruPreviewGroup = document.getElementById(
            "noAssetBaruPreviewGroup"
        );
        const noAssetBaruPreviewInput = document.getElementById(
            "no_asset_baru_preview"
        );
        const noAssetBaruWarning =
            document.getElementById("noAssetBaruWarning");
        let currentBarangData = null;

        const updateNoAssetBaruPreview = async () => {
            const perusahaanTujuanOption =
                perusahaanTujuanSelect.options[
                    perusahaanTujuanSelect.selectedIndex
                ];
            ...
            try {
                ...
                if (data.success) {
                    const nomorSeri = data.nomor_seri;
                    const singkatanPT =
                        perusahaanTujuanOption.dataset.singkatan;
                    const singkatanJenis =
                        currentBarangData.jenis_barang.singkatan;
        ...
        perusahaanTujuanSelect.addEventListener(
            "change",
            updateNoAssetBaruPreview
        );
        window.setCurrentBarangForSerahTerima = (barang) => {
    ...
    if (exportExcelButton) {
        exportExcelButton.addEventListener("click", function () {
            const hasDataRows = document.querySelector('#productTableRowsContainer .products-row[data-id]');
            if (!hasDataRows) {
            ...
            const jenisBarangCheckboxes = document.querySelectorAll(
                '#filter_jenis_barang_container input[type="checkbox"]:checked'
            );
            const jenisBarang = Array.from(jenisBarangCheckboxes).map(
                (cb) => cb.value
            );

            const params = new URLSearchParams();
            if (keyword) {
                params.append("search_no_asset", keyword);
            }
            if (perusahaan) {
                params.append("filter_perusahaan", perusahaan);
            }
    ...
    function openModalElement(modalElement, formToReset = null) {
        if (modalElement) {
            ...
            if (formToReset) {
                ...
                formToReset
                    .querySelectorAll(".form-control.is-invalid")
    ...
    function displayValidationErrorsOnForm(errors, formElement) {
        ...
        for (const field in errors) {
            ...
            if (errorElement && inputElement) {
                ...
                console.warn(
                    `Elemen error untuk field '${field}' tidak ditemukan.`
                );
            }
        }
    }
    ...
    function performRealtimeSearch(page = 1) {
        ...
        productTableRowsContainer.innerHTML =
            '<div class="products-row"><div class="product-cell" style="text-align:center; flex-basis:100%; padding: 40px;">Memuat data... <i class="fas fa-spinner fa-spin"></i></div></div>';
                    ...
                    responseData.data.forEach((barang) => {
                        ...
                        tableRowsHtml += `
                            <div class="products-row" data-id="${barang.id}">
                                ...
                                <div class="product-cell cell-perusahaan" data-label="Perusahaan" title="${escapeHtml(
                                    barang.perusahaan_nama || "N/A"
                                )}">${escapeHtml(
                            barang.perusahaan_nama || "N/A"
                        )}</div>
                                <div class="product-cell cell-jenis-barang" data-label="Jenis Barang" title="${escapeHtml(
                                    barang.jenis_barang || "N/A"
                                )}">${escapeHtml(
                            barang.jenis_barang || "N/A"
                        )}</div>
                                <div class="product-cell cell-no-asset" data-label="No Asset" title="${escapeHtml(
                                    barang.no_asset || ""
                                )}">${escapeHtml(barang.no_asset || "")}</div>
                                <div class="product-cell cell-merek" data-label="Merek" title="${escapeHtml(
                                    barang.merek || ""
                                )}">${escapeHtml(barang.merek || "")}</div>
                                <div class="product-cell cell-tgl-pengadaan" data-label="Tgl. Pengadaan">${formatDate(
                                    barang.tgl_pengadaan
                                )}</div>
                                <div class="product-cell cell-serial-number" data-label="Serial Number" title="${escapeHtml(
                                    barang.serial_number || ""
                                )}">${escapeHtml(
                            barang.serial_number || ""
                        )}</div>
                                <div class="product-cell cell-lokasi" data-label="Lokasi">${escapeHtml(
                                    barang.lokasi || "N/A"
                                )}</div>
                                <div class="product-cell cell-aksi">
                                    <button class="action-btn-table detail-btn-table-js" data-id="${
                                        barang.id
                                    }" title="Detail Aset"><i class="fas fa-info-circle"></i><span>Detail</span></button>
                                    ${superAdminButtons}
                                </div>
                            </div>`;
                    });
    ...
    function updateInventorySummary(summaryData) {
        ...
        document
            .querySelectorAll(".summary-box .summary-box-count")
            .forEach((el) => {
                el.textContent = "0";
            });
        ...
        for (const namaJenis in summaryData) {
            ...
            const slug = namaJenis
                .toLowerCase()
                .replace(/ \/ /g, "-")
                .replace(/ /g, "-");

            const matchingBoxes = document.querySelectorAll(
                `.summary-box[data-type="${slug}"]`
            );

            if (matchingBoxes.length > 0) {
                matchingBoxes.forEach((box) => {
                    const countElement =
                        box.querySelector(".summary-box-count");
                    if (countElement) {
    ...
    function checkAndUpdateFilterStates() {
        if (!filterPerusahaanSelect || !resetFilterInMenuBtn || !filterToggleButton) return;
        ...
        const keywordAktif = mainSearchInput ? mainSearchInput.value.trim() !== "" : false;

        const isAnyFilterActive = perusahaanAktif || jenisBarangAktif || keywordAktif;

        resetFilterInMenuBtn.disabled = !isAnyFilterActive;
        if (isAnyFilterActive) {
            filterToggleButton.classList.add("filter-active");
        } else {
            filterToggleButton.classList.remove("filter-active");
        }
    }
    ...
    if (closeAddAssetModalBtn)
        closeAddAssetModalBtn.addEventListener("click", () =>
            closeModalElement(addAssetModal)
        );
    if (cancelAddAssetModalBtn)
        cancelAddAssetModalBtn.addEventListener("click", () =>
            closeModalElement(addAssetModal)
        );
    setupSmartModalClosure(addAssetModal, () =>
        closeModalElement(addAssetModal)
    );
    ...
    if (closeDetailModalButtonElement)
        closeDetailModalButtonElement.addEventListener(
            "click",
            closeDetailModal
        );
    ...
    if (cancelSerahTerimaBtn)
        cancelSerahTerimaBtn.addEventListener("click", closeSerahTerimaModal);
    ...
    if (closeUserHistoryBtn)
        closeUserHistoryBtn.addEventListener("click", closeUserHistoryModal);
    setupSmartModalClosure(userHistoryModalElement, closeUserHistoryModal);

    document.addEventListener("keydown", (e) => {
        if (e.key === "Escape") {
            if (addAssetModal.classList.contains("show"))
                closeModalElement(addAssetModal);
            if (detailModalOverlayElement.style.display === "flex")
                closeDetailModal();
            if (serahTerimaAsetModalElement.style.display === "flex")
                closeSerahTerimaModal();
            if (userHistoryModalElement.style.display === "flex")
                closeUserHistoryModal();
        }
    });

    setupSmartModalClosure(
        serahTerimaAsetModalElement,
        (el) => (el.style.display = "none")
    );
    if (serahTerimaStatusSelect) {
        serahTerimaStatusSelect.addEventListener(
            "change",
            handleSerahTerimaStatusChange
        );
    }
    if (closeSerahTerimaBtn)
        closeSerahTerimaBtn.addEventListener("click", () => {
            if (serahTerimaAsetModalElement)
                serahTerimaAsetModalElement.style.display = "none";
        });
    if (cancelSerahTerimaBtn)
        cancelSerahTerimaBtn.addEventListener("click", () => {
            if (serahTerimaAsetModalElement)
                serahTerimaAsetModalElement.style.display = "none";
        });
    ...
    if (filterMenu && filterToggleButton) {
        document.addEventListener("click", function (event) {
            ...
            const isClickOnFilterToggleButton = filterToggleButton.contains(
                event.target
            );

            if (
                filterMenu.classList.contains("active") &&
                !isClickInsideFilterMenu &&
                !isClickOnFilterToggleButton
            ) {
                filterMenu.classList.remove("active");
            }
        });
    ...
    function setActiveSidebarLink() {
        ...
        if (!document.querySelector(".sidebar .sidebar-list-item.active")) {
            const dashboardLinkEl = document.querySelector(
                `.sidebar-list-item a[href="${window.PAGE_DATA.urls.dashboardIndex}"]`
            );
            if (
                dashboardLinkEl &&
                (window.location.pathname === "/" ||
                    window.location.pathname ===
                        new URL(dashboardLinkEl.href).pathname)
            ) {
