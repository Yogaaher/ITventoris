...
document.addEventListener('DOMContentLoaded', () => {
        ...
        fetch(url, {
            ...
            .then(data => {
                if (data.table_html && data.pagination) {
                    ...
                    console.error('Data table_html atau pagination tidak ditemukan dari server.');
                }
            })
            .catch(error => {
                alert(`Gagal memuat data. Pesan Error: ${error.message || 'Lihat console untuk detail.'}`);
            });
    };
    ...
    const setupModal = (id = null) => {
        ...
        document.getElementById('modalTitle').innerHTML = isEditMode ? conf.modalTitleEdit : conf.modalTitleAdd;
        dataForm.action = isEditMode ? conf.updateUrl + id : conf.storeUrl;
        document.getElementById('formMethod').value = isEditMode ? 'PUT' : 'POST';
        ...
        document.getElementById('singkatan').maxLength = conf.modalSingkatanMaxlength;
    ...
    document.getElementById('openAddModalButton').addEventListener('click', () => setupModal());
