document.addEventListener("DOMContentLoaded", () => {
    ...
    const ui = {
        ...
        paginationButtons: document.getElementById(
    ...
    const renderContent = (items) => {
        ...
        items.forEach((item, index) => {
            ...
            if (currentTab === "serahTerima") {
                row.innerHTML = renderSerahTerimaRow(
                    item,
                    item.original_row_number || rowNumber
                );
            } else if (currentTab === "mutasi") {
    ...
    const renderSerahTerimaRow = (surat, number) => {
        ...
        if (isAdmin || isSuperAdmin) {
            actions =
                `<a href="${config.serahTerima.downloadUrl}/${surat.id}" class="action-btn-table download-btn" title="Download PDF" target="_blank" rel="noopener noreferrer"><i class="fas fa-download"></i></a>` +
                actions;
        }
    ...
    const renderMutasiRow = (mutasi, number) => {
        ...
        <div class="product-cell" style="flex:1.5">${new Date(
        ...
        links.forEach((link) => {
            ...
            if (link.url) {
                btn.addEventListener("click", (e) => {
                    e.preventDefault();
                    state.currentPage = new URL(link.url).searchParams.get(
                        "page"
                    );
                    fetchData();
                });
    ...
    const setupUI = () => {
        ...
        const applyTheme = () => {
            ...
            if (ui.modeSwitch)
                ui.modeSwitch.classList.toggle("active", theme === "light");
        };
        ...
        const initializeSidebar = () => {
            ...
            if (ui.burgerMenu)
                ui.burgerMenu.classList.toggle("active", !isCollapsed);
        };
        ...
        document.body.addEventListener("click", (event) => {
            if (
                ...
                if (ui.mobileBurgerMenu)
                    ui.mobileBurgerMenu.classList.remove("active");
            }
        });
    ...
    const openModalForEdit = (surat) => {
        ...
        document.getElementById("suratModalTitle").textContent =
            "Edit Surat Serah Terima";
        document.getElementById("suratModalIcon").className =
            "fas fa-edit modal-header-icon";
        ...
        document.getElementById("jabatan_penerima").value =
        ...
        document.getElementById("jabatan_pemberi").value =
            surat.jabatan_pemberi;
        ...
        searchBarang.value =
            surat.barang?.serial_number || surat.barang?.no_asset || "";
    ...
    const showDetailModal = (surat) => {
        ...
        const iconClassFromDb =
            surat.barang?.jenis_barang?.icon || "fas fa-question-circle";
        ...
        <div class="detail-section">
            ...
            <dl class="detail-item"><dt>No. Surat:</dt><dd>${
                surat.no_surat || "N/A"
            }</dd></dl>
            <dl class="detail-item"><dt>Tgl. Dibuat:</dt><dd>${formatDate(
                surat.created_at
            )}</dd></dl>
            <dl class="detail-item"><dt>Penanggung Jawab:</dt><dd>${
                surat.penanggung_jawab || "N/A"
            }</dd></dl>
            <dl class="detail-item" style="align-items: flex-start;"><dt>Keterangan:</dt><dd style="white-space: pre-wrap;">${
                surat.keterangan || "-"
            }</dd></dl>
        </div>
        ...
        <div class="detail-section">
            ...
            <div class="detail-barang-box">
                ...
                <div class="detail-barang-info">
                    ...
                    <div class="subheader">${
                        surat.barang?.jenis_barang?.nama_jenis || "N/A"
                    }</div>
                </div>
            </div>
            <hr style="border: none; border-top: 1px solid var(--table-border); margin: 1.5rem 0;">
            <dl class="detail-item"><dt>Perusahaan:</dt><dd>${
                surat.barang?.perusahaan?.nama_perusahaan || "N/A"
            }</dd></dl>
            <dl class="detail-item"><dt>No Asset:</dt><dd>${
                surat.barang?.no_asset || "N/A"
            }</dd></dl>
            <dl class="detail-item"><dt>Tgl. Pengadaan:</dt><dd>${formatDate(
                surat.barang?.tgl_pengadaan
            )}</dd></dl>
            <dl class="detail-item"><dt>Serial Number:</dt><dd>${
                surat.barang?.serial_number || "N/A"
            }</dd></dl>
        </div>
    `;
    ...
    const showDetailMutasiModal = (mutasi) => {
        ...
        const iconClass =
            mutasi.barang?.jenis_barang?.icon || "fas fa-question-circle";

        document.getElementById(
            "detailMutasiIcon"
        ).className = `detail-barang-icon ${iconClass}`;
        document.getElementById("detailMutasiMerek").textContent =
            mutasi.barang?.merek || "N/A";
        document.getElementById("detailMutasiJenis").textContent =
            mutasi.barang?.jenis_barang?.nama_jenis || "N/A";
        document.getElementById("detailMutasiSN").textContent =
            mutasi.barang?.serial_number || "N/A";
        document.getElementById("detailMutasiTanggal").textContent = formatDate(
            mutasi.tanggal_mutasi
        );

        document.getElementById("detailMutasiAssetLama").textContent =
            mutasi.no_asset_lama || "N/A";
        document.getElementById("detailMutasiPerusahaanLama").textContent =
            mutasi.perusahaan_lama?.nama_perusahaan || "N/A";
        document.getElementById("detailMutasiPenggunaLama").textContent =
            mutasi.pengguna_lama || "N/A";

        document.getElementById("detailMutasiAssetBaru").textContent =
            mutasi.no_asset_baru || "N/A";
        document.getElementById("detailMutasiPerusahaanBaru").textContent =
            mutasi.perusahaan_baru?.nama_perusahaan || "N/A";
        document.getElementById("detailMutasiPenggunaBaru").textContent =
            mutasi.pengguna_baru || "N/A";
    ...
    document.querySelector(".nav-tabs").addEventListener("click", (e) => {
        ...
        document.querySelectorAll(".nav-link").forEach((link) => {
            ...
            link.style.borderBottomColor = isActive
                ? "var(--action-color)"
                : "transparent";
        });
        ...
        document.querySelectorAll(".tab-pane").forEach((pane) => {
            pane.style.display =
                pane.id === `${currentTab}-tab-pane` ? "block" : "none";
        });
    ...
    const openAddSuratModalButton = document.getElementById(
        "openAddSuratModalButton"
    );
    if (openAddSuratModalButton) {
        openAddSuratModalButton.addEventListener("click", () => {
            ...
            document.getElementById("suratModalIcon").className =
                "fas fa-plus-square modal-header-icon";
    ...
    document.addEventListener("keydown", (e) => {
        if (e.key === "Escape") {
            ...
            if (
                ui.detailSuratModal &&
                ui.detailSuratModal.classList.contains("show")
            ) {
    ...
    ui.searchBarangInput.addEventListener("input", (e) => {
        ...
        state.searchBarangDebounce = setTimeout(() => {
            ...
            ui.barangInfoBox.style.display = "block";
            ui.barangInfoBox.className = "info-box-loading";
            ui.barangInfoBox.innerHTML = `<p><i class="fas fa-spinner fa-spin"></i> Mencari aset...</p>`;
            
            fetch(`${PAGE_DATA.urls.serahTerima.findBarang}?query=${encodeURIComponent(query)}`)
                .then((response) => response.json())
                .then((barang) => {
                    ui.barangInfoBox.classList.remove("info-box-loading");
                    ui.barangIdInput.value = "";
                    ui.submitSuratBtn.disabled = true;

                    if (barang && barang.id) {
                        ui.barangInfoBox.className = "info-box-success";
                        ui.barangInfoBox.innerHTML = `
                            <h4><i class="fas fa-check-circle"></i> Aset Ditemukan</h4>
                            <p><strong>Merek:</strong> ${barang.merek || 'N/A'}</p>
                            <p><strong>No Asset:</strong> ${barang.no_asset || 'N/A'}</p>
                            <p><strong>Serial Number:</strong> ${barang.serial_number || 'N/A'}</p>
                        `;
                        ...
                        ui.barangInfoBox.innerHTML = `
                            ...
                            <p>Pastikan Serial Number atau No. Asset yang Anda masukkan sudah benar.</p>
                        `;
                    }
                })
                .catch(error => {
                    ...
                    ui.barangInfoBox.className = "info-box-error";
                    ui.barangInfoBox.innerHTML = `<h4><i class="fas fa-times-circle"></i> Terjadi Kesalahan</h4><p>Gagal terhubung ke server. Coba lagi nanti.</p>`;
                });
        }, 500);
    });
    ...
    ui.suratForm.addEventListener("input", () => {
        ...
        if (hasChanged) {
            ...
            ui.submitSuratBtn.disabled = true;
            ui.submitSuratBtn.setAttribute(
                "data-tooltip",
    ...
    ui.suratForm.addEventListener("submit", (e) => {
        ...
        if (isUpdateMode) {
            ...
            url = PAGE_DATA.urls.serahTerima.store;
        }
            ...
            .then(({ status, body }) => {
                if (status === 200 || status === 201) {
                    ...
                    Object.keys(body.errors).forEach((key) => {
                        ...
                        if (key === "barang_id") {
                            inputElement =
                                document.getElementById("search_barang");
                        }
                    ...
                    const firstInvalidElement =
                        ui.suratForm.querySelector(".is-invalid");
                    if (firstInvalidElement) {
                        const scrollTarget =
                            firstInvalidElement.closest(".form-section") ||
                            firstInvalidElement;
                        const modalBody =
                            ui.suratModal.querySelector(".modal-body");
