...
<div class="products-row" data-user-id="{{ $user->id }}">
    ...
    <div class="product-cell cell-username" data-label="Username" title="{{ $user->name }}">{{ $user->name }}</div>
    <div class="product-cell cell-email" data-label="Email" title="{{ $user->email }}">{{ $user->email }}</div>
    <div class="product-cell cell-role" data-label="Role">{{ $user->role ?? 'User' }}</div>
    <div class="product-cell cell-tanggal-dibuat" data-label="Tanggal Dibuat">{{ $user->created_at->format('d-m-Y') }}</div>
